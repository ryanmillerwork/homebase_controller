                  ...
                  >
                    <template v-slot:running>
                      <div
                        ...
                        @click="toggleTask(true, device.device)"
                        :style="{
                          color: getRunningStatus(device.device)
                            ? 'green'
                            : 'black',
                        }"
                      >
                        ...
                        <q-icon
                          name="play_arrow"
                          :color="
                            getRunningStatus(device.device) ? 'green' : 'black'
                          "
                        ></q-icon>
                      </div>
                    </template>
                    <template v-slot:stopped>
                      <div
                        class="toggle-content full-width full-height q-pa-none cursor-pointer"
                        @click="toggleTask(false, device.device)"
                        :style="{
                          color: !getRunningStatus(device.device)
                            ? 'red'
                            : 'black',
                      ...
                      >
                        ...
                        <q-icon
                          name="stop"
                          :color="
                            !getRunningStatus(device.device) ? 'red' : 'black'
                          "
                        ></q-icon>
                      </div>
                    </template>
...
const perfStats = computed(() => {
  const groupedData = perfStatsFull.value.reduce((acc, entry) => {
    const { hb, status_type, pc, rt, trials, aborts, sys, protocol, variant } =
      entry;
    ...
    if (sys === "*" && protocol === "*" && variant === "*") {
      ...
      acc[hb][status_type] = {
        ...
        correct: `${Math.round(pc * (trials - aborts * trials))} / ${Math.round(
          trials - aborts * trials
        )} (${Math.round(pc * 100)}%)`, // Custom format for correct
        rt: Math.round(rt),
...
function toggleTask(value, hb) {
  // Find the IP address for the device in commStatus
  const ipAddress =
    commStatus.value.find((entry) => entry.device === hb)?.address || "";
  ...
  if (!ipAddress) {
    $q.notify({
      type: "warning",
      message: `No IP address found for device ${hb}`,
      timeout: 5000, // Dismiss after 5 seconds
    });
    return;
  }

  // Check if there is a non-empty animal status for the given hb
  const animalStatus = statusData.value.find(
    (entry) => entry.hb === hb && entry.status_type === "animal"
  );

  if (!animalStatus || animalStatus.status_val.trim() === "") {
    $q.notify({
      type: "warning",
      message: `Must select an animal!`,
      timeout: 5000, // Dismiss after 5 seconds
    });
    return;
  }
...
function getTimeSinceLastEvent(device) {
  const entry = statusData.value.find((item) => {
    return item.hb === device && item.status_type === "last_resp";
  });
  ...
  const lastEventTimeUTC = new Date(entry.server_time).getTime();
...
function addNewAnimal() {
  ...
  sendMessage("AddAnimal", "", newAnimalName.value);
...
function getBatteryLevel(hb) {
  ...
  const maxEntry = statusData.value.find(
    (entry) => entry.hb === hb && entry.status_type === "24v_volt_max"
  );

  if (currentEntry && maxEntry) {
    const currentV = parseFloat(currentEntry.status_val);
    const maxV = parseFloat(maxEntry.status_val);
...
function getBatteryIcon(hb) {
  ...
  if (currentEntry && maxEntry) {
    ...
    if (current < 0.1) {
      ...
      else if (batteryPercent > 76) return "battery_charging_80";
      else if (batteryPercent > 64) return "battery_charging_60";
      else if (batteryPercent > 52) return "battery_charging_50";
      else if (batteryPercent > 40) return "battery_charging_30";
      else if (batteryPercent > 28) return "battery_charging_20";
