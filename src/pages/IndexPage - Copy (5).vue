          ...
          <q-card>
            <q-card-section>
              ...
              <div class="q-mt-md row items-center">
                <!-- Toggle Button Column: 8/12 width -->
                ...
                <!-- Reset Button Column: 4/12 width -->
...
function resetTask(hb) {
  console.log(`Reset task for hb ${hb}`);
  const commEntry = commStatus.value.find((entry) => entry.device === hb);
  const ipAddress = commEntry ? commEntry.address : "";
...
function getBatteryIcon(hb) {
  ...
  } else if (voltage > 28.5 && current <= 0.1) {
    // reaches 28.5 when discharging after 28 mins
    return "battery_full"; // works
  } else if (voltage > 27 && current > 0.1) {
    return "sym_o_battery_charging_60"; //
  } else if (voltage > 28.2 && current <= 0.1) {
    // reaches 28.2 when discharging after 28 mins
