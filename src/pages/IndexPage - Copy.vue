...
function handleUserSelection(val, hb, type) {
  ...
  if (type === "system") {
    sendMessage("esscmd", "192.168.4.101", `::ess::load_system ${val}`);
...
function toggleTask(value, hb) {
  ...
  if (value) {
    sendMessage("esscmd", "192.168.4.101", "::ess::start");
  } else {
    sendMessage("esscmd", "192.168.4.101", "::ess::stop");
  }
}

function getLinkStatus(hb) {
  // Placeholder function to determine whether to show 'link' or 'link_off'
  // Currently always returns false (i.e., 'link_off')
  return false;
}

function getLinkColor(hb) {
  // Placeholder function to determine the color of the link icon
