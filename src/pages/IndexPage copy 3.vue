          ...
          <q-card>
            <q-card-section>
              ...
              <div class="row justify-content-end">
                <!-- Button to open parameter picker -->
                <q-btn
                  outline
                  color="grey-8"
                  class="bold-icon full-width"
                  style="width: 100%; height: 50px"
                  label="Specify task parameters"
...
// temporaroy for debugging only
// watch(
//   listboxOptions,
//   (newVal) => {
//     console.log("IndexPage listboxOptions changed:", newVal);
//   },
//   { deep: true }
// );

// const availableValues = {
//   subject: ["momo", "toto", "fifi", "gogo", "wawa", "gigi", "baba", "popo"],
//   system: ["emcalib", "search", "planko"],
//   protocol: ["training"],
//   variant: ["single", "jitter", "zero_one"],
// };

// const variantInfoRow = computed(() => {
//   return (
//     statusData.value.find(
//       (item) =>
//         item.host === device.address && item.status_type === "variant_info"
//     ) || null
//   );
// });
...
//     // The big string from row.status_value might look like:
//     //   "loader_proc basic_planko loader_args {50 1 0.1 { ball_jitter_x 10 ... }}
//     //    loader_arg_names {nr nplanks wrong_catcher_alpha params}
//     //    loader_arg_options {nr {{50 50} {100 100} {200 200}} ...}"

//     // 3A) Parse the parameter names
//     const namesMatch = row.status_value.match(/loader_arg_names\s*\{([^}]+)\}/);
//     if (namesMatch) {
//       paramNames.value = namesMatch[1].trim().split(/\s+/);
//     } else {
//       paramNames.value = [];
//     }

//     // 3B) Parse the parameter options
//     const optionsMatch = row.status_value.match(
//       /loader_arg_options\s*\{([^}]+)\}/
//     );
//     if (optionsMatch) {
//       paramOptions.value = parseLoaderArgOptions(optionsMatch[1]);
//     } else {
//       paramOptions.value = {};
//     }

//     // 3C) Parse the current parameter selections
//     const argsMatch = row.status_value.match(/loader_args\s*\{([^}]+)\}/);
//     if (argsMatch) {
//       // e.g. "50 1 0.1 { ball_jitter_x 10 ... }"
//       const tokens = splitByTopLevelSpaces(argsMatch[1]);
//       paramNames.value.forEach((name, i) => {
//         selectedParams.value[name] = tokens[i] || "";
//       });
//     } else {
//       selectedParams.value = {};
//     }
...
function parseLoaderArgOptions(raw) {
  ...
  while ((match = re.exec(raw)) !== null) {
    ...
    console.log("optionsBlock:", optionsBlock);

    let extractedPairs;

    if (paramName === "targ_color") {
      ...
      while ((outerMatch = outerPairRegex.exec(optionsBlock)) !== null) {
        ...
        const matches = block.match(/{\s*(\w+)\s*({[^}]*})\s*}/);
        if (matches) {
...
function saveTaskParams() {
  ...
  console.log("User selections for", selectedHost.value, selectedParams.value);
  showParamsDialog.value = false;

  // If you need to send them to your server:
  // sendMessage('esscmd', selectedHost.value, buildSaveCommand(selectedParams.value))
}

// function parseLoaderArgOptions(raw) {
//   const result = {};
//   // Example raw: "nr {{50 50} {100 100} {200 200}} nplanks {{1 1}} ..."
//   // We'll match paramName + the block of {{...}}
//   const re = /(\S+)\s*\{\{([^}]+)\}\}/g;
//   let match;
//   while ((match = re.exec(raw)) !== null) {
//     const paramName = match[1];
//     const inside = match[2]; // e.g. "50 50} {100 100} {200 200"
//     const subMatches = inside.match(/\{([^}]+)\}/g) || [];
//     const extractedValues = subMatches.map((m) => {
//       // Each chunk looks like "{50 50}", so we take the first token as 'val'
//       return m.replace(/[{}]/g, "").trim().split(/\s+/)[0];
//     });
//     result[paramName] = extractedValues;
//   }
//   return result;
// }
