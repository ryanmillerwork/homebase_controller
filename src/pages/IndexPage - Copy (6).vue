...
<script setup>
import { ref, onMounted, onUnmounted, watch, h } from "vue"; // MIT License
import useWebSocket, { sendMessage } from "src/composables/useWebSocket";
import { useQuasar } from "quasar"; // used for notifications             // MIT License
...
//   const now = Date.now();
//   const lastEventTime = commStatus.value[device]?.lastEventTime || now;
//   return now - lastEventTime;
// }

// function getTimeSinceLastEvent(device) {
//   // 1 minute = 60,000 milliseconds
//   // 43 seconds = 43,000 milliseconds
//   // Total = 103,000 milliseconds
//   return 300000; // Always return 1 minute and 43 seconds in milliseconds
// }
...
function getTimeSinceLastEvent() {
  const lastEventTime = new Date("2024-10-29T12:44:00").getTime(); // 12:12 PM on 10/29/24
  return currentTime.value - lastEventTime; // Difference in milliseconds
}
...
function getSecondsValue(device) {
  ...
  return (seconds / 60) * 100; // change it to percentage of a minute
}
...
.custom-arrow-icon {
  ...
  width: 4px; /* Adjust width if needed */
