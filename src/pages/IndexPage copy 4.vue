          ...
          <q-card>
            <q-card-section>
              ...
              <q-dialog v-model="showSystemParamsDialog" persistent>
                <q-card style="min-width: 300px">
                  <q-card-section class="text-h6"
                    >System Parameters</q-card-section
                  >
                  <q-card-section>
                    <div v-if="systemParamNames.length">
                      <div
                        v-for="(name, idx) in systemParamNames"
                        :key="idx"
                        class="q-mb-md"
                      >
                        <q-input
                          v-model="selectedSystemParams[name]"
                          :label="name"
                          filled
                          dense
                          emit-value
...
const systemParamNames = ref([]); // e.g. ["nr", "nplanks", ...]
const selectedSystemParams = ref({}); // e.g. { nr: 50, nplanks: 1, ... }
